...
def place_magnet(arm: Arm, magnet: Magnet, x: float, y: float,
...
def move_magnet(arm: Arm, magnet: Magnet, x: float, y: float,
                orientation: float = 0.0) -> None:
...
def grip_magnet(arm: Arm, magnet: Magnet) -> None:
...
def release_magnet(arm: Arm, magnet: Magnet, x: float, y: float,
                   orientation: float = 0.0) -> None:
...
def flip_paper(arm: Arm) -> None:
    ...
    arm.move_offset_world(0, 0, config.FLIP_PAPER_CLEARANCE)

    current_tcp = arm.current_tcp_pose()
    ...
    arm.rotate_absolute(point_down)
    ...
    arm.rotate_absolute(rotate_wrist) # rotate the wrist joint by 180 degrees to flip the paper
    ...
    arm.rotate_absolute(point_forwards)
...
def fold_arc(
    ...
    n_steps : int
        ...
        time.  Default 8.
    fold_percent : float
        Amount at which to stop rotating and just move
    """
...
def grip_crease_tool(arm: Arm, x: float, y: float, z: float, grip_angle: float) -> None:
...
def return_creaser_tool(arm: Arm, x: float, y: float, z: float, grip_angle: float) -> None:
