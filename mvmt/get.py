...
def track_board_corners():
    # this function cycles the left arm through the 4 corners of the board
    ...
    # add precise end affector rotation
...
def rotate_wrist(arm_control, pos, amt):
    raise NotImplementedError("this doesn't work because of IK")
    # this method attempts to rotate the wrist a set amount (doesn't work rn)
...
def main_movement():
    # this code resets and initalizes the right arm position to bottom-right corner of page, then grabs the bottom right corner to prepare for folding.
    # initialize right arm pos, and move it back a bit so we don't hit anything
    pos = right_receive.getActualTCPPose()
    pos[0] += 15/100
    right_control.moveL(pos)
    
    # now initialize so the arm goes the the bottom right of the board
    pos = right_arm['bottom_right']
    #pos[2] += 10/100 # temp line for debug, do stuff above the board and not directly on it
    # offset of board height
    pos[2] += 7.65/100
    right_control.moveL(pos)
    ...
    right_gripper.close()
    input('right arm in initial position. press <ENTER> to advance.')
...
def calculate_circle_motion():
    # this code "calculates" movement through a half circle with a given radius and angle change
    # fold
    radius = 9.50/100
    angle = -1 * math.pi/4
...
def estimated_circular_motion():
    # Moves the right arm through a set of half-circle positions
    arc_poses = [
    ...
    mvmt_params = [0.5,0.5,0]
    poses_with_mvmt_params = [pose+mvmt_params for pose in arc_poses]
    
    right_control.moveL(poses_with_mvmt_params)


def cirular_motion_test():
